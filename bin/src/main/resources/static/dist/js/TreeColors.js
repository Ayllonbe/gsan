  ...
  function TreeColors(preset) {
    ...
    function assignHue(node, range, level) {
      ...
      if(level === 0) { // node is the root
		...
		//console.log(node.data.name + "--" +range + " -- " + chromaStart + " -- " + chromaDelta + " -- " + level );
        setColor(node, {h: (range[1] +range[0]) /2, c: chromaStart + chromaDelta * level, l: luminanceStart + luminanceDelta * level});
      }
