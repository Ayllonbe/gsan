	...
	public void printHierarchy(OWLReasoner reasoner, OWLClass clazz, String source)
		...
		for(OWLClass r:onto){ // For each owlclass child of owl:Thing
			...
			if(sino == false){ // False : subontology
				 ...
				 */
				if(EntitySearcher.getAnnotations(r.getIRI(), ontology,df.getRDFSLabel()).size()>0){
				...
				for (OWLClass pere : list1) { // For each owl class in list1
					if (!pere.isBottomEntity() ) { // if that owl class is not a owl:Nothing
						...
						for(OWLAnnotationAssertionAxiom oa : EntitySearcher.getAnnotationAssertionAxioms(pere.getIRI(), ontology))
							{
							for(OWLAnnotation o : oa.getAnnotations(df.getRDFSLabel())){
								labelaxioms.add(((OWLLiteral)o.getValue()).getLiteral());
							}
								
							}
						for(OWLAnnotation oa : EntitySearcher.getAnnotations(pere.getIRI(), ontology,df.getRDFSLabel()))
							{
						 ...
						 * The method changetype is to transform the owl class to GO id
						 */
						iT.is_a.ancestors.addAll(changetype(reasoner.getSuperClasses(pere,false).getFlattened(),ids));
						iT.is_a.parents.addAll(changetype(reasoner.getSuperClasses(pere,true).getFlattened(),ids));
						iT.is_a.descendants.addAll(changetype(reasoner.getSubClasses(pere,false).getFlattened(),ids));
						iT.is_a.childrens.addAll(changetype(reasoner.getSubClasses(pere,true).getFlattened(),ids));
						...
						for(OWLSubClassOfAxiom subclsaxiom : ontology.getSubClassAxiomsForSubClass(pere) ){
							...
							for(OWLObjectProperty reg:subclsaxiom.getSuperClass().getObjectPropertiesInSignature()){
								namereg = new String();
								for(OWLAnnotation oa : EntitySearcher.getAnnotations(reg.getIRI(), ontology,ids))
								...
								case "part_of":
									for(OWLClass cpof : subclsaxiom.getSuperClass().getClassesInSignature()){
										for(OWLAnnotation oa : EntitySearcher.getAnnotations(cpof.getIRI(), ontology,ids))
											{
...
//		}
	...
	public static List<String> changetype(Set<OWLClass> set, OWLAnnotationProperty id){
		...
		for(OWLClass c : set){ // For each owl class
			...
			for(OWLAnnotation oa : EntitySearcher.getAnnotations(c.getIRI(), ontology,id))
